write.csv(result,file=paste(path,'/report/report',Sys.Date(),'.csv',sep=''),row.names=FALSE)
...
for (i in 1:length(stockId)){
	stock <- stockId[i]
	write.csv(get(stock),file=paste(path,'/report/results/',stock,'.csv',sep=''),row.names=FALSE)
}
